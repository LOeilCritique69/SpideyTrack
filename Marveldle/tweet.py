...
# ════════════════════════════════════════════════════════════════
# 3. METTRE À JOUR LE TOP 5 (CLASSEMENT ULTIME)
# ════════════════════════════════════════════════════════════════
def update_top5():
    ...
    # 1. Collecter TOUS les scores de TOUS les persos depuis le début
    for char in CHARACTERS:
        ...
        with open(char["results_file"], "r", encoding="utf-8") as f:
            ...
            for day_key, val in data.items():
                ...
                # On crée une légende propre pour la carte
    ...
    if not all_candidates:
        print("ℹ️ Aucun score trouvé pour le classement.")
        return

    # 2. Trier par score (exacts) PUIS par partiels (en cas d'égalité)
    # On prend les 5 meilleurs
    sorted_top = sorted(
        all_candidates, 
        key=lambda x: (x["score"], x["partial"]), 
        reverse=True
    )[:5]

    # 3. Préparer le bloc JS pour le HTML
    js_lines = []
    for i, entry in enumerate(sorted_top):
        comma = "," if i < len(sorted_top) - 1 else ""
        # On échappe les guillemets si besoin
        js_lines.append(
            f'    {{ screenshot: "{entry["screenshot"]}", caption: "{entry["caption"]}", day: {entry["day"]} }}{comma}'
        )
    
    # On cherche le pattern TOP3 ou TOP5 pour être flexible
    import re
    with open(html_file, "r", encoding="utf-8") as f:
        html_content = f.read()

    # Pattern qui cherche "const TOP" suivi de n'importe quel chiffre
    pattern = re.compile(r'const TOP\d\s*=\s*\[.*?\];', re.DOTALL)
    new_js_block = f"const TOP5 = [\n" + "\n".join(js_lines) + "\n];"

    if pattern.search(html_content):
        new_html = pattern.sub(new_js_block, html_content)
        with open(html_file, "w", encoding="utf-8") as f:
            f.write(new_html)
        print(f"✅ Classement TOP 5 mis à jour (Scanné {len(all_candidates)} parties).")
    else:
        print("❌ Variable 'const TOP5' introuvable dans le HTML.")
